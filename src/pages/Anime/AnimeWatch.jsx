...
export default function AnimeWatch() {
    ...
    return (
        <div className="flex min-h-screen bg-gradient-to-r from-[#182c18] to-[#012903] text-white font-mincho">
            <Sidebar />
            <main className="flex-1 ml-16 md:ml-60 px-6 py-10 flex flex-col lg:flex-row gap-6">
                <div className="flex-1 space-y-6">
                    <div>
                        <h1 className="text-2xl font-bold mb-2">{item.title}</h1>
                        {selectedEpisode && <h2 className="text-lg text-white/70">Episode {selectedEpisode.number}: {selectedEpisode.title}</h2>}
                    </div>

                    <div ref={videoContainerRef} className={`group relative pt-[56.25%] w-full overflow-hidden rounded-xl shadow-lg bg-black ${isFullscreen ? 'fixed top-0 left-0 w-screen h-screen z-50 rounded-none' : ''}`}>
                        <div id="youtube-player" className="absolute top-0 left-0 w-full h-full"></div>
                        {playerReady && (
                            <button onClick={toggleFullscreen} className="absolute top-4 right-4 bg-black/50 hover:bg-black/70 text-white p-2 rounded-full z-20 transition opacity-0 group-hover:opacity-100">
                        ...
                        {playerReady && (activeJapaneseSubtitle) && (
                            <div className="absolute top-4 left-1/2 -translate-x-1/2 w-fit max-w-[90%] p-3 text-center bg-black/40 rounded-lg">
                                {activeJapaneseSubtitle && (
                                    <p className="text-yellow-300 text-xl font-bold cursor-pointer hover:underline" onClick={() => handleJapaneseSubtitleClick(activeJapaneseSubtitle.text)}>
                ...
                {showWordDetailsPanel && (
                    <div className="w-full lg:w-80 bg-black/20 rounded-xl p-4 shadow-lg flex-shrink-0 max-h-[calc(100vh-120px)] overflow-y-auto border border-white/10">
                        <div className="flex justify-between items-center mb-3">
                           ...
                           <button onClick={() => setShowWordDetailsPanel(false)} className="text-2xl hover:text-red-500">&times;</button>
                        </div>
                        ...
                        {!loadingWordData && !errorWordData && selectedWordData && (
                            <div className="space-y-4">
                                {Array.isArray(selectedWordData) && selectedWordData.map((wordInfo, idx) => (
                                    <div key={idx} className="pb-2 border-b border-gray-700 last:border-b-0">
                                        ...
                                        <p className="text-sm">Meaning: {wordInfo.meaning || 'Not found'}</p>
                                    </div>
                                ))}
                            </div>
                        )}
