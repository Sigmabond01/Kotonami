    ...
    export default function Podcast() {
      ...
      useEffect(() => {
        const fetchVideo = async () => {
          ...
          try {
            setLoading(true);
            const response = await axios.get(`http://localhost:3004/api/podcasts/${slug}`);
            setVideo(response.data);
          } catch (err) {
            console.error("Failed to fetch details:", err);
            setError("Could not find vid!");
          } finally {
            setLoading(false);
          }
        };
    ...
    try {
      ...
      if (loading) {
        return <div className="text-white p-10">Loading Podcast...</div>;
      }
      if (error || !video) {
